import { CORDIS_AMQP_SYMBOLS, CORDIS_REDIS_SYMBOLS, PatchedAPIPresence, Patcher } from '@cordis/util';
...
const presenceUpdate: Handler<GatewayPresenceUpdate> = async (data, service, cache) => {
  if (data.guild_id) {
    const existing = await cache.get<PatchedAPIPresence>(CORDIS_REDIS_SYMBOLS.cache.presences(data.guild_id), data.user.id);
