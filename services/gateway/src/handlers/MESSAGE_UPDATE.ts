import { CORDIS_AMQP_SYMBOLS, CORDIS_REDIS_SYMBOLS, PatchedAPIGuild, PatchedAPIMessage, Patcher } from '@cordis/util';
...
const messageUpdate: Handler<GatewayMessageUpdateDispatch['d']> = async (data, service, cache) => {
  const existing = await cache.get<PatchedAPIMessage>(CORDIS_REDIS_SYMBOLS.cache.messages(data.channel_id), data.id);
