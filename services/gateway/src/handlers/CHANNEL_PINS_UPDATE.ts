...
const channelPinsUpdate: Handler<GatewayChannelPinsUpdateDispatch['d']> = async (data, service, cache) => {
  const guild = data.guild_id ? await cache.get<APIGuild>('guilds', data.guild_id) : null;
  const foundChannel = guild
    ? guild.channels?.find(e => e.id === data.channel_id)
    : await cache.get<APIChannel>('dm_channels', data.channel_id);
  ...
  if (channel) {
    service.publish({ guild: guild ?? undefined, channel, lastPinTimestamp: data.last_pin_timestamp }, 'channelPinsUpdate');
  }
};
