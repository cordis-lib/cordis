import { Patcher, CORDIS_REDIS_SYMBOLS, CORDIS_AMQP_SYMBOLS, PatchedGuild, PatchedChannel } from '@cordis/util';
...
export const channelUpdate: Handler<APIChannel> = async (data, service, cache) => {
  if (data.guild_id) {
    ...
    if (guild) {
      const found = await cache.get<PatchedChannel>(CORDIS_REDIS_SYMBOLS.cache.channels(guild.id), data.id);
    ...
    const found = await cache.get<PatchedChannel>(CORDIS_REDIS_SYMBOLS.cache.channels(), data.id);
    const { data: n, old: o } = found ? Patcher.patchChannel(data, found) : Patcher.patchChannel(data);
