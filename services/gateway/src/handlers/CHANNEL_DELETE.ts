import { Patcher, CORDIS_REDIS_SYMBOLS, CORDIS_AMQP_SYMBOLS, PatchedAPIChannel, PatchedAPIGuild } from '@cordis/util';
import { APIChannel } from 'discord-api-types';
import { Handler } from '../Handler';

const channelDelete: Handler<APIChannel> = async (data, service, cache) => {
  const guild = data.guild_id ? await cache.get<PatchedAPIGuild>(CORDIS_REDIS_SYMBOLS.cache.guilds, data.guild_id) : null;
  if (guild) {
    const old = await cache.get<PatchedAPIChannel>(CORDIS_REDIS_SYMBOLS.cache.channels(guild.id), data.id);
    ...
    const old = await cache.get<PatchedAPIChannel>(CORDIS_REDIS_SYMBOLS.cache.channels(), data.id);
    const { data: channel } = old ? Patcher.patchChannel(data, old) : Patcher.patchChannel(data);
