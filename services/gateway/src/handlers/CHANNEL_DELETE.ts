import { Patcher, CORDIS_REDIS_SYMBOLS, CORDIS_AMQP_SYMBOLS, PatchedChannel, PatchedGuild } from '@cordis/util';
...
const channelDelete: Handler<APIChannel> = async (data, service, cache) => {
  ...
  if (guild) {
    const old = await cache.get<PatchedChannel>(CORDIS_REDIS_SYMBOLS.cache.channels(guild.id), data.id);
    ...
    const old = await cache.get<PatchedChannel>(CORDIS_REDIS_SYMBOLS.cache.channels(), data.id);
    const { data: channel } = old ? Patcher.patchChannel(data, old) : Patcher.patchChannel(data);
