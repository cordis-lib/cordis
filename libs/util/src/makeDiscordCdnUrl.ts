...
export const makeDiscordCdnUrl = (root: string, options?: ImageOptions | null) => {
  ...
  if (!['webp', 'png', 'jpg', 'gif'].includes(format)) {
    throw new CordisUtilTypeError('badImageFormat', format);
  }

  if (size && ![16, 32, 64, 128, 256, 512, 1024, 2048].includes(size)) {
    throw new CordisUtilRangeError('badImageSize', size);
  }
